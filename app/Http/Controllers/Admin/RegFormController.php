...
class RegFormController extends Controller
{
    ...
    public function admin_ListRegForms()
    {
        //
        $reform = RegForm::all();
     ...
     public function admin_SaveRegForm(Request $request)
     {
         //
            ...
            try {
                DB::beginTransaction(); 
                   ...
                   $regform =  RegForm::create([
                    ...
                    'reg_status' => 'que',
                    'slug' => time().Str::slug($request->input('cand_fname')).Str::random(40)
                   // Str::slug($request->input('setting_name'))  
                ]);

                    $cand_slug = $reform->slug;
     ...
     public function user_CompleteReg(Request $request, $admission_slug, $cand_slug){
                //Retrieve datas from AdmissionReg table
                 $admission = AdmissionReg::where('slug', $admission_slug)->first();
                ...
                //Retrieve datas from RegForm table
                 ...
                 if($feeRequired == 1 && $payment = 'paid' ){
                    ...
                    if($examRequired ==1 && $candScore >= $cutOff){
    ...
    public function admin_EditRegForm(Request $request, $slug)
    {
        //
     ...
     public function admin_UpdateRegForm(Request $request, $slug)
     {
         //
     ...
     public function admin_DeleteRegForm(Request $request, $slug)
     {
         //
